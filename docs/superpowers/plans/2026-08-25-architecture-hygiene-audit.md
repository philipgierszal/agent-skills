...
Use NUL-safe Git output and parse bytes on NUL boundaries. Inventory the union of HEAD (`git ls-tree`), all index stages (`git ls-files --stage`), and non-ignored untracked paths. Sort paths without rewriting legal path bytes and serialize surrogate-escaped POSIX bytes as JSON escapes. Record `tracked_at_head`, `head_entry`, `git_object`, `index_stages`, `index_status` (`tracked`, `absent`, or `conflicted`), independent `worktree_status` (`present` or `missing`), `git_mode`, `kind`, `size_bytes`, and SHA-256 where content exists. For mode `160000`, also record initialization, checked-out HEAD, and submodule dirty state. Compute a canonical `inventory_digest` over the complete file metadata so same-HEAD worktree, index, conflict, and submodule drift is detectable. Distinguish actual symlinks from Git symlinks materialized as regular files. Never descend through path text returned by another shell.
